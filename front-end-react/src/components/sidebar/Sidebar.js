...
export class Sidebar extends React.Component {
    ...
    componentDidMount(){
        ...
        document.addEventListener("click", (ev) => {
            ...
            if(!container.contains(ev.target) && ev.target.className !== 'genre-on' && ev.target.className !== 'favorite-show'){
                this.hideSidebar();
            }
        });
    }
...
function Favorites(props){
    let favorites = props.favorites.map((favorite) => {
        return(
            <div key={favorite.title} className="favorite-show" onClick={() => props.removeFavorite(favorite)}>
                {favorite.title}
            </div>
        )
    })
